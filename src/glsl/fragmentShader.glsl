...
float d2Groups[groupNum * groupSize];
...
int aNodeIndex(int i) { return int(rawGroups[groupSize * i + cylinderSize * 0 + 1]); }// where the node is pointing
int bNodeIndex(int i) { return int(rawGroups[groupSize * i + cylinderSize * 1 + 1]); }// -//-
...
vec4 d2Group(in vec3 pos, int i) {
	...
	// Shape A
	if (aShapeType(i) == 0) {
		d2a = d2Groups[aNodeIndex(i)];
	}
	else if (aShapeType(i) == 1) { d2a = d2Cube    (pos, aShapePos(i), aShapeScale(i)); }
	else if (aShapeType(i) == 2) { d2a = d2Sphere  (pos, aShapePos(i), aShapeRadius(i)); }
	else if (aShapeType(i) == 3) { d2a = d2Cylinder(pos, aShapePos(i), aShapeScale(i), aShapeRadius(i)); }
	// Shape B
	if (bShapeType(i) == 0) {
		d2b = d2Groups[bNodeIndex(i)];
	}
	else if (bShapeType(i) == 1) { d2b = d2Cube    (pos, bShapePos(i), bShapeScale(i)); }
	else if (bShapeType(i) == 2) { d2b = d2Sphere  (pos, bShapePos(i), bShapeRadius(i)); }
	else if (bShapeType(i) == 3) { d2b = d2Cylinder(pos, bShapePos(i), bShapeScale(i), bShapeRadius(i)); }

	vec4 f = Combine(d2a, d2b, aShapeClr(i), bShapeClr(i), grpOpType(i), grpK(i));
